  ...
  {#each items as t (keyOf(t))}
    {#if t.href}
      <a
        class="tag {t.class ?? ''}"
        href={t.href}
        data-fs={fontPx(t.weight, t.label).toFixed(1)}
        style="font-size: {fontPx(t.weight, t.label).toFixed(1)}px; opacity: {opacity(
          t.weight,
        ).toFixed(2)};"
        title={t.title ?? String(t.weight)}>{label(t.label)}</a
      >
    {:else}
      <span
        class="tag {t.class ?? ''}"
        ...
        title={t.title ?? String(t.weight)}>{label(t.label)}</span
      >
    {/if}
  {/each}
</div>
...
<style>
  ...
  .tag {
    ...
    text-align: center;
    font-weight: 700;
    /* Theme with CSS custom properties; sensible defaults out of the box. */
    color: var(--otc-color, currentColor);
  ...
  a.tag:hover,
  a.tag:focus-visible {
    color: var(--otc-hover-color, #2563eb);
    opacity: 1 !important;
    text-decoration: none;
  }
